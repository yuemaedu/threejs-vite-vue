...
onMounted(() => {
  ...
  const textureFlare0 = new TextureLoader()
      .load(lensflare0)

  const textureFlare3 = new TextureLoader()
      .load(lensflare3)
  const flareColor = new Color(0xffaacc)
  const lensFlare = new Lensflare()
  // LensflareElement
//   texture - 炫光的材质.
//   ize - (可选) 炫光的尺寸大小，单位是像素
//   distance - (可选) 光源到相机的距离，为0时在光源处，为1时在相机处，0 - 1取值
//   color - (可选) 炫光的颜色
//   blending - (可选) 炫光的混合模式- 默认为 THREE.NormalBlending
  // 加入了五个光晕效果，第一个用来模拟太阳，其余的用来模拟光源，其中光晕的大小、距离、颜色、材质，都可以根据需要调节。
  lensFlare.addElement(new LensflareElement(textureFlare0, 500, 0, flareColor));
  lensFlare.addElement(new LensflareElement(textureFlare3, 60, 0.6, flareColor));
  lensFlare.addElement(new LensflareElement(textureFlare3, 100, 0.7, flareColor));
  lensFlare.addElement(new LensflareElement(textureFlare3, 60, 0.9, flareColor));
  lensFlare.addElement(new LensflareElement(textureFlare3, 70, 1, flareColor));
